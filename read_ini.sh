...
read_ini()
{
	...
	#local SWITCH_SHOPT=""
	local ${VARNAME_PREFIX}__ALL_SECTIONS=""
	local ${VARNAME_PREFIX}__ALL_VARS=""
	pollute_bash
...
######################					
		...
		then
##############
			VARNAME="${VARNAME_PREFIX}$(tr '[:lower:]' '[:upper:]' <<< ${VAR:0:1})${VAR:1}"
#			VARNAME=${VARNAME_PREFIX}__${VAR//./_}
		else
			VARNAME="${VARNAME_PREFIX}$(tr '[:lower:]' '[:upper:]' <<< ${SECTION:0:1})${SECTION:1}$(tr '[:lower:]' '[:upper:]' <<< ${VAR:0:1})${VAR:1}"
#			VARNAME=${VARNAME_PREFIX}__${SECTION}__${VAR//./_}
##############
		fi
		eval "${INI_ALL_VARNAME}=\"\${${INI_ALL_VARNAME}# } ${VARNAME}\""
##############
#		echo "${INI_ALL_VARNAME}"
##############
		if [[ "${VAL}" =~ ^\".*\"$  ]]
		then
